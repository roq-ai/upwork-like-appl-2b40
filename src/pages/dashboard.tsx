...
import { useRoqPlatformUserProfiles, useBugReportCount, useTestCaseCount, useProjectCount } from 'lib/roq';

import { BugReportListPage as ShowCaseList } from 'pages/bug-reports';

function DashboardPage() {
  const { session } = useSession();

  const bugReportStat = useBugReportCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Bug Reports',
      result: bugReportStat?.data,
      isLoading: bugReportStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`bug-reports`} passHref legacyBehavior>
                  <Button
